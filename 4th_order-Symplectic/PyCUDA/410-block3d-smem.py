...
__global__ void update_h(int nx, int ny, int nz, float cl, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *chx, float *chy, float *chz) {
	...
	if (tx < 2) {
		sy[tx][ty][tz+2] = ey[idx+(tx-2)];
		sz[tx][ty+2][tz] = ez[idx+(tx-2)];
	}
	if (ty < 2) {
		sx[tx][ty][tz+2] = ex[idx+(ty-2)*nx];
		sz[tx+2][ty][tz] = ez[idx+(ty-2)*nx];
	}
	if (tz < 2) {
		sx[tx][ty+2][tz] = ex[idx+(tz-2)*nx*ny];
		sy[tx+2][ty][tz] = ey[idx+(tz-2)*nx*ny];
	}
	if (tx == Dx-1) {
...
/*
	if( j>1 && j<ny-1 && k>1 && k<nz-1 ) 
		hx[idx] -= cl*chx[idx]*( 27*( sz[tx+2][ty+2][tz] - sz[tx+2][ty+1][tz] - sy[tx+2][ty][tz+2] + sy[tx+2][ty][tz+1] )
								- ( sz[tx+2][ty+3][tz] - sz[tx+2][ty][tz] - sy[tx+2][ty][tz+3] + sy[tx+2][ty][tz] ) );
...
*/
	if( j>1 && j<ny-1 && k>1 && k<nz-1 ) 
		hx[idx] -= cl*chx[idx]*( 27*( sz[tx+2][ty+2][tz] - sz[tx+2][ty+1][tz] - ey[idx] + ey[idx-nx*ny] )
								- ( ez[idx+nx] - ez[idx-2*nx] - ey[idx+nx*ny] + ey[idx-2*nx*ny] ) );
	if( i>1 && i<nx-1 && k>1 && k<nz-1 ) 
		hy[idx] -= cl*chy[idx]*( 27*( sx[tx][ty+2][tz+2] - ex[idx-nx*ny] - ez[idx] + ez[idx-1] )
								- ( ex[idx+nx*ny] - ex[idx-2*nx*ny] - ez[idx+1] + ez[idx-2] ) );
	if( i>1 && i<nx-1 && j>1 && j<ny-1 ) 
		hz[idx] -= cl*chz[idx]*( 27*( ey[idx] - ey[idx-1] - sx[tx][ty+2][tz+2] + sx[tx][ty+1][tz+2] )
