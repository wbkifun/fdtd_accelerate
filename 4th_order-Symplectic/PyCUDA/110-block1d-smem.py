...
__global__ void update_h(int nx, int ny, int nz, float cl, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *chx, float *chy, float *chz) {
	...
	float* sx = (float*) s;
	float* sy = (float*) &sx[blockDim.x+2];
	float* sz = (float*) &sy[blockDim.x+3];
	...
	if( i>1 && i<nx-1 && k>1 && k<nz-1 ) 
		hy[idx] -= cl*chy[idx]*( 27*( sx[tx] - ex[idx-nx*ny] - sz[tx] + sz[tx-1] )
								- ( ex[idx+nx*ny] - ex[idx-2*nx*ny] - sz[tx+1] + sz[tx-2] ) );
	if( i>1 && i<nx-1 && j>1 && j<ny-1 ) 
		hz[idx] -= cl*chz[idx]*( 27*( sy[tx] - sy[tx-1] - sx[tx] + ex[idx-nx] )
								- ( sy[tx+1] - sy[tx-2] - ex[idx+nx] + ex[idx-2*nx] ) );
}
...
__global__ void update_e(int nx, int ny, int nz, float dl, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *cex, float *cey, float *cez) {
	...
	if( j>0 && j<ny-2 && k>0 && k<nz-2 ) 
		ex[idx] += dl*cex[idx]*( 27*( hz[idx+nx] - sz[tx] - hy[idx+nx*ny] + sy[tx] )
								- ( hz[idx+2*nx] - hz[idx-nx] - hy[idx+2*nx*ny] + hy[idx-nx*ny] ) );
	if( i>0 && i<nx-2 && k>0 && k<nz-2 ) 
		ey[idx] += dl*cey[idx]*( 27*( hx[idx+nx*ny] - sx[tx] - sz[tx+1] + sz[tx] )
								- ( hx[idx+2*nx*ny] - hx[idx-nx*ny] - sz[tx+2] + sz[tx-1] ) );
	if( i>0 && i<nx-2 && j>0 && j<ny-2 ) 
		ez[idx] += dl*cez[idx]*( 27*( sy[tx+1] - sy[tx] - hx[idx+nx] + sx[tx] )
								- ( sy[tx+2] - sy[tx-1] - hx[idx+2*nx] + hx[idx-nx] ) );
}
...
if __name__ == '__main__':
	...
	update_h.prepare("iiifPPPPPPPPP", block=Db, shared=(3*tpb+6)*4)
	update_e.prepare("iiifPPPPPPPPP", block=Db, shared=(3*tpb+6)*4)
