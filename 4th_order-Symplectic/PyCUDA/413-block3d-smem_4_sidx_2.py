...
__global__ void update_h(int nx, int ny, int nz, float cl, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *chx, float *chy, float *chz) {
	...
	int sxi = tx + ty*Dx + tz*Dx*sDy;
	int syi = tx + ty*sDx + tz*sDx*Dy;
	int szi = tx + ty*sDx + tz*sDx*sDy;

	extern __shared__ float s[];
	float* sx = (float*) &s[2*(Dx*(sDy+1)+1)];
	float* sy = (float*) &sx[Dx*sDy*sDz-2*(Dx*(sDy+1)+1)+2*(sDx*Dy+1)];
	float* sz = (float*) &sy[sDx*Dy*sDz-2*(sDx*Dy+1)+2*(sDx+1)];
...
*/
	...
	if( j>1 && j<ny-1 && k>1 && k<nz-1 ) 
		hx[idx] -= cl*chx[idx]*( 27*( sz[szi] - ez[idx-nx] - sy[syi] + ey[idx-nx*ny] )
...
if __name__ == '__main__':
	...
	update_h.prepare("iiifPPPPPPPPP", block=Db, shared=(( 16*(8+3)*(4+3) + (16+3)*8*(4+3) + (16+3)*(8+3)*4 )*4))
