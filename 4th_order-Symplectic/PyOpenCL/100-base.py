...
kernels =""" 
__kernel void update_h(const int nx, const int ny, const int nz, const float cl, 
		__global const float *ex, __global const float *ey, __global const float *ez, 
		__global float *hx, __global float *hy, __global float *hz, 
		__global const float *chx, __global const float *chy, __global const float *chz) {
...
__kernel void update_e(const int nx, const int ny, const int nz, const float dl, 
		__global float *ex, __global float *ey, __global float *ez, 
		__global const float *hx, __global const float *hy, __global const float *hz, 
		__global const float *cex, __global const float *cey, __global const float *cez) {
	int idx = get_global_id(0);
	int k = idx/(nx*ny);
	int j = (idx - k*nx*ny)/nx;
	int i = idx%nx;
...
__kernel void update_src(const int nx, const int ny, const int nz, const float tn, 
		__global float *f) {
	int idx = get_global_id(0);
