...
def real2discrete(ds, *real_value):
	"""
	이름 : real2discrete
	기능 : 이 함수는 프로젝트 파일에서 real 공간 값으로 선언된 것을 전산모사에서 계산을 할 수 있도록 discrete 한 공간 값으로 변환해준다. 따라서 주어지는 인자는 한 cell의 최소 단위인 dx가 주어지고, 변환하고자 하는 real 값들을 넣어준다.
	인자 : dx, real_vals
	반환 : discrete_val

	보충 : 들어오는 real 값 개수에 맞추어 결과 discrete 값의 개수도 반환된다. 단지 한 개가 아닌 여러개 인자인 경우 tuple로 반환이 된다.
	"""
...
def calc_with_list(value1, operator ,value2):  ## 테스트 코드 필요함.
	...
	if operator not in ['+','-','*','/','**']:
		raise Exception, 'There is no mached operator. Please use +, -, *, /, **.'
	if value1_ty is not ListType and value2_ty is not ListType:
		raise Exception, 'Both variables are not List. Just use %s Operator' % operator
