...
"""
 ...
 File Name : conformal_structure_2d.py
...
def ep_eff_conformal(ep1, ep2, d1, d2):
	return d1*ep1 + d2*ep2
	...
	for i in xrange(Nx):
		...
		for structure in structures[1:]:
			...
			if grid_y1 != grid_y2:
				...
				line_arrays[y_axis][i,j1] = ep_eff_conformal(bg_epr, structure.epr, d1, 1-d1)
				line_arrays[y_axis][i,j2] = ep_eff_conformal(structure.epr, bg_epr, d2, 1-d2)
	...
	for j in xrange(Ny):
		...
		for structure in structures[1:]:
			...
			if grid_x1 != grid_x2:
				...
				line_arrays[x_axis][i1,j] = ep_eff_conformal(bg_epr, structure.epr, d1, 1-d1)
				line_arrays[x_axis][i2,j] = ep_eff_conformal(structure.epr, bg_epr, d2, 1-d2)
...
if __name__ == '__main__':
	...
	for ep1, ep2 in ep_list:
		for d1 in d1_list: 
			...
			ep_eff = ep_eff_conformal(ep1, ep2, d1, 1-d1)
			ep_eff2 = ep_eff_conformal_numerical(ep1, ep2, d1, 1-d1)
	...
	d1, d2 = 0.2, 0.3
	d3 = 1 - (d1 + d2)
	ep_eff = ep_eff_conformal(ep1, ep2, d1, d2)
	ep_eff1 = ep_eff_conformal(ep_eff, ep3, d1+d2, d3)

	ep_eff = ep_eff_conformal(ep2, ep3, d2, d3)
	ep_eff2 = ep_eff_conformal(ep1, ep_eff, d1, d2+d3)
