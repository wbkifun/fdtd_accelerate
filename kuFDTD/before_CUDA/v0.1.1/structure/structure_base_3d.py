...
"""
 ...
 File Name : structure_base_3d.py
...
Define the geometrical functions for 3-dimensional structure.
...
def make_matrix_vector(*point):
    vector = []
    for pt in xrange( len(point) ):
        V = sc.zeros((3,1), 'f')
        V[0,0], V[1,0], V[2,0] = pt[0], pt[1], pt[2]
        vector.append( sc.matrix(V) )
    if len(vector) == 1:
        return vector[0]
    else:
        return vector
...
def matrix_vector2point(*vector):
    point = []
    for vec in len(vector):
        point.append( (vec[0,0], vec[1,0], vec[2,0]) )
    if len(point) == 1:
        return point[0]
    else:
        return point
...
def rotate(point, rotation_angles):
    ...
    return matrix_vector2point( Rz*Ry*Rx*X )
