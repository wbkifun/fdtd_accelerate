...
from core.cpml_core import update_cpml_3d
from core.cpml_core import update_cpml_2update_1base
from core.cpml_core import update_cpml_1update_2base
...
class Cpml:
	...
	def alloc_psi_arrays(self, in_out_field, field):
		dimension = self.space.dimension
		Np = self.number_pml_cells
		
		if dimension == '3D':
			Nx, Ny, Nz = self.space.number_cells
			...
			Nx, Ny = self.space.number_cells
			if in_out_field == 'in_field':
				psi_yx = sc.zeros((2*Np, Ny+1), 'f')
				psi_zx = sc.zeros((2*Np, Ny+1), 'f')
				psi_zy = sc.zeros((Nx+1, 2*Np), 'f')
				psi_xy = sc.zeros((Nx+1, 2*Np), 'f')
			elif in_out_field == 'out_field':
				psi_yx = sc.zeros((2*Np, Ny+2), 'f')
				psi_zx = sc.zeros((2*Np, Ny+2), 'f')
				psi_zy = sc.zeros((Nx+2, 2*Np), 'f')
				psi_xy = sc.zeros((Nx+2, 2*Np), 'f')

			psi_xz = None
			psi_yz = None
			
			if self.space.dimension == '2DEz':
				if field == 'e_field':
					psi_yx = None
					psi_xy = None
				elif field == 'h_field':
	...
	def allocate_pml_arrays(self):
		...
		if self.space.dimension == '3D':
			if self.space.grid_opt == 'efaced':
				self.psi_e = self.alloc_psi_arrays('out_field', 'e_field')        
				self.psi_h = self.alloc_psi_arrays('in_field', 'h_field')        
			elif self.space.grid_opt == 'hfaced':
				self.psi_e = self.alloc_psi_arrays('in_field', 'e_field')        
				self.psi_h = self.alloc_psi_arrays('out_field', 'h_field')        
