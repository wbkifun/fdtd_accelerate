...
class DielectricIf( Dielectric ):
	...
	def prepare_kernels( s ):
		fpath = '%s/trial/core/pass_xy0cell-args-smem.cu' % base_dir
		mod = cuda.SourceModule( file( fpath,'r' ).read() )
		s.update_e = mod.get_function("update_e")
		s.update_h = mod.get_function("update_h")

		Db = ( s.tpb_main, 1, 1 )
		s.update_e.prepare( "iiiiiPPPPPPPPP", block=Db, shared=s.ns_main )
		s.update_h.prepare( "iiiiiPPPPPP", block=Db, shared=s.ns_main )
