...
class DielectricIf( Dielectric ):
	...
	def allocate_main_in_dev( s ):
		...
		s.devHx = cuda.mem_alloc( s.bytes1 )
		s.devHy = cuda.mem_alloc( s.bytes1 )
		s.devHz = cuda.mem_alloc( s.bytes1 )
	...
	def set_kernel_parameters( s ):
		...
		#s.bpg_main = s.calc_bpg( (s.Nx-1)*s.Ny*s.Nz, s.tpb_main )
		s.ns_main = ( 2*(s.tpb_main+1)+s.tpb_main )*s.bytes_f


	def prepare_kernels( s ):
		fpath = '%s/trial/core/pass_h_x0cell-smem-1if.cu' % base_dir
		mod = cuda.SourceModule( file( fpath,'r' ).read() )
		s.update_e = mod.get_function("update_e")
		s.update_h = mod.get_function("update_h")

		Db = ( s.tpb_main, 1, 1 )
		s.update_e.prepare( "iiiPPPPPPPPP", block=Db, shared=s.ns_main )
		s.update_h.prepare( "iiiPPPPPP", block=Db, shared=s.ns_main )


	def updateE( s ):
		s.update_e.prepared_call( \
				( s.bpg_main, 1 ), \
				sc.int32( s.size ), s.kNz, sc.int32(s.Ny*s.Nz), \
	...
	def updateH( s ):
		s.update_h.prepared_call( \
				( s.bpg_main, 1 ), \
				sc.int32( (s.Nx-1)*s.Ny*s.Nz ), s.kNz, sc.int32(s.Ny*s.Nz), \
				s.devEx, s.devEy, s.devEz, \
				s.devHx, s.devHy, s.devHz )
