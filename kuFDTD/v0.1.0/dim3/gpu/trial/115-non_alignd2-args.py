...
class DielectricIf( Dielectric ):
	...
	def set_kernel_parameters( s ):
		...
		s.bpg_main = s.calc_bpg( (s.Nx-2)*(s.Ny-2)*(s.Nz-2), s.tpb_main )
		s.ns_main = ( 2*(s.tpb_main+1)+s.tpb_main )*s.bytes_f


	def prepare_kernels( s ):
		fpath = '%s/trial/core/non_aligned2-args.cu' % base_dir
	...
	def updateE( s ):
		s.update_e.prepared_call( \
				( s.bpg_main, 1 ), \
				s.kNz, sc.int32(s.Ny*s.Nz), sc.int32(s.Nz-2), sc.int32( (s.Ny-2)*(s.Nz-2) ), sc.int32( (s.Nx-2)*(s.Ny-2)*(s.Nz-2) ), \
