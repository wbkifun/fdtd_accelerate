...
class DielectricIf( Dielectric ):
	...
	def initmem_main_in_dev( s ):
		...
		tpb = 512
		bpg = s.calc_bpg( (s.Nx-1)*(s.Ny-1)*(s.Nz-1), tpb )
		N = sc.int32( (s.Nx-1)*(s.Ny-1)*(s.Nz-1) )
		Db = (tpb,1,1)
		Dg = (bpg,1)
		...
		print 'surplus= ', tpb*bpg - (s.Nx-1)*(s.Ny-1)*(s.Nz-1)
	...
	def prepare_kernels( s ):
		fpath = '%s/trial/core/non_aligned.cu' % base_dir
	...
	def updateE( s ):
		s.update_e.prepared_call( \
				( s.bpg_main, 1 ), \
				sc.int32( (s.Nx-2)*(s.Ny-1)*(s.Nz-1) ), s.kNy, s.kNz, \
	...
	def updateH( s ):
		s.update_h.prepared_call( \
				( s.bpg_main, 1 ), \
				sc.int32( (s.Nx-1)*(s.Ny-1)*(s.Nz-1) ), s.kNy, s.kNz, \
				s.devEx, s.devEy, s.devEz, \
				s.devHx, s.devHy, s.devHz )
