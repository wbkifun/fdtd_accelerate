...
class DielectricIf( Dielectric ):
	...
	def allocate_main_in_dev( s ):
		s.devEx = cuda.mem_alloc( s.bytes1 )
		s.devEy = cuda.mem_alloc( s.bytes1 )
		s.devEz = cuda.mem_alloc( s.bytes1 )
	...
	def prepare_kernels( s ):
		fpath = '%s/trial/core/non_aligned-Nx+1.cu' % base_dir
