...
class DielectricIf( Dielectric ):
	...
	def set_kernel_parameters( s ):
		...
		s.bpg_main = s.calc_bpg( (s.Nx-1)*(s.Ny-1)*(s.Nz-1), s.tpb_main )
		s.ns_main = ( 2*(s.tpb_main+1)+s.tpb_main )*s.bytes_f


	def prepare_kernels( s ):
		fpath = '%s/trial/core/non_aligned-args.cu' % base_dir
	...
	def updateE( s ):
		s.update_e.prepared_call( \
				( s.bpg_main, 1 ), \
				s.kNz, sc.int32(s.Ny*s.Nz), sc.int32(s.Nz-1), sc.int32( (s.Ny-1)*(s.Nz-1) ), sc.int32( (s.Nx-1)*(s.Ny-1)*(s.Nz-1) ), \
