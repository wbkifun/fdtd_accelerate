...
//	psi2[pidx] = rcmbE[pk]*psi2[pidx] + rcmaE[pk]*( hx[tk+1] - hx[tk] );
	...
	//if ( hy[tk] - H.y[idx] != 0 || hx[tk] - H.x[idx] != 0 || hy[tk+1] - H.y[idx+1] != 0 || hx[tk+1] - H.x[idx+1] != 0 ) {
	int i = idx/(N.y*N.z);
	j = (idx - i*N.y*N.z)/N.z;
	int k = idx%N.z;
	if ( k == 29 || k == 30 || k == 31 ) {
		...
		printf("\thy[%d]=%g, hy[%d]=%g, H.y[%d]=%g, H.y[%d]=%g\n", tk+1, hy[tk+1], tk, hy[tk], idx+1, H.y[idx+1], idx, H.y[idx]); 
		//for ( int l=0; l<blockDim.x; l++ ) printf("hy[%d]=%g, hx[%d]=%g\n", l, hy[l], l, hx[l]);
	}
}
...
int main() {
	...
	size_t Ns_pmlz = sizeof(float)*( 2*(TPB+TPB/Npml) );
