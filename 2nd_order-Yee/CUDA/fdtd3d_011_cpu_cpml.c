...
/*
	...
	for ( tstep=1; tstep<=TMAX; tstep++ ) {
		updateE( N, E, H, CE );
		updateCPMLx( N, E, H, CE, 1, Npml+1, 1, N.y-1, 1, N.z-1, psixE.y, psixE.z, bE, aE, 1, 0 );
		updateCPMLx( N, E, H, CE, N.x-(Npml+1), N.x-1, 1, N.y-1, 1, N.z-1, psixE.y, psixE.z, bE, aE, 1, 1 );
		updateCPMLy( N, E, H, CE, 1, N.x-1, 1, Npml+1, 1, N.z-1, psiyE.z, psiyE.x, bE, aE, 1, 0 );
		updateCPMLy( N, E, H, CE, 1, N.x-1, N.y-(Npml+1), N.y-1, 1, N.z-1, psiyE.z, psiyE.x, bE, aE, 1, 1 );
		updateCPMLz( N, E, H, CE, 1, N.x-1, 1, N.y-1, 1, Npml+1, psizE.x, psizE.y, bE, aE, 1, 0 );
		updateCPMLz( N, E, H, CE, 1, N.x-1, 1, N.y-1, N.z-(Npml+1), N.z-1, psizE.x, psizE.y, bE, aE, 1, 1 );

		updateSrc( N, E, tstep );

		updateH( N, E, H );
		updateCPMLx( N, E, H, CE, 1, Npml+1, 1, N.y, 1, N.z, psixH.y, psixH.z, bH, aH, 0, 0 );
		updateCPMLx( N, E, H, CE, N.x-Npml, N.x, 1, N.y, 1, N.z, psixH.y, psixH.z, bH, aH, 0, 1 );
		updateCPMLy( N, E, H, CE, 1, N.x, 1, Npml+1, 1, N.z, psiyH.z, psiyH.x, bH, aH, 0, 0 );
		updateCPMLy( N, E, H, CE, 1, N.x, N.y-Npml, N.y, 1, N.z, psiyH.z, psiyH.x, bH, aH, 0, 1 );
		updateCPMLz( N, E, H, CE, 1, N.x, 1, N.y, 1, Npml+1, psizH.x, psizH.y, bH, aH, 0, 0 );
		updateCPMLz( N, E, H, CE, 1, N.x, 1, N.y, N.z-Npml, N.z, psizH.x, psizH.y, bH, aH, 0, 1 );
