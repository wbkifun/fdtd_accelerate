...
float ***makeArray(int Nx, int Ny, int Nz) {
	...
	for (i=0; i<Nz*Ny*Nx; i++) f[0][0][i] = 0;
...
void updateE(N3 N, P3F3 E, P3F3 H, P3F3 CE) {
	...
	i=N.x-1;
	for (j=0; j<N.y-1; j++) for (k=0; k<N.z-1; k++) 
		E.x[i][j][k] += CE.x[i][j][k]*(H.z[i][j+1][k] - H.z[i][j][k] - H.y[i][j][k+1] + H.y[i][j][k]);

	j=N.y-1;
	for (i=0; i<N.x-1; i++) for (k=0; k<N.z-1; k++) 
		E.y[i][j][k] += CE.y[i][j][k]*(H.x[i][j][k+1] - H.x[i][j][k] - H.z[i+1][j][k] + H.z[i][j][k]);
...
int main() {
	...
	P3F3 E, H, CE;
	E.x = makeArray(N.x, N.y, N.z);
	E.y = makeArray(N.x, N.y, N.z);
	E.z = makeArray(N.x, N.y, N.z);
	H.x = makeArray(N.x, N.y, N.z);
	H.y = makeArray(N.x, N.y, N.z);
	H.z = makeArray(N.x, N.y, N.z);
	CE.x = makeArray(N.x, N.y, N.z);
	CE.y = makeArray(N.x, N.y, N.z);
	CE.z = makeArray(N.x, N.y, N.z);
	...
	psixE.y = makeArray(2*Npml, N.y, N.z);
	psixE.z = makeArray(2*Npml, N.y, N.z);

	psiyE.z = makeArray(N.x, 2*Npml, N.z);
	psiyE.x = makeArray(N.x, 2*Npml, N.z);

	psizE.x = makeArray(N.x, N.y, 2*Npml);
	psizE.y = makeArray(N.x, N.y, 2*Npml);

	psixH.y = makeArray(2*Npml, N.y, N.z);
	psixH.z = makeArray(2*Npml, N.y, N.z);

	psiyH.z = makeArray(N.x, 2*Npml, N.z);
	psiyH.x = makeArray(N.x, 2*Npml, N.z);

	psizH.x = makeArray(N.x, N.y, 2*Npml);
	psizH.y = makeArray(N.x, N.y, 2*Npml);
	...
	//for ( tstep=1; tstep<=500; tstep++) {
		updateE(N, E, H, CE);
		updateCPMLx(N, E, H, CE, 0, Npml, 0, N.y-1, 0, N.z-1, psixE.y, psixE.z, bE, aE, 1, 0);
		updateCPMLx(N, E, H, CE, N.x-11, N.x-1, 0, N.y-1, 0, N.z-1, psixE.y, psixE.z, bE, aE, 1, 1);
		updateCPMLy(N, E, H, CE, 0, N.x-1, 0, Npml, 0, N.z-1, psiyE.z, psiyE.x, bE, aE, 1, 0);
		updateCPMLy(N, E, H, CE, 0, N.x-1, N.y-11, N.y-1, 0, N.z-1, psiyE.z, psiyE.x, bE, aE, 1, 1);
		updateCPMLz(N, E, H, CE, 0, N.x-1, 0, N.y-1, 0, Npml, psizE.x, psizE.y, bE, aE, 1, 0);
		updateCPMLz(N, E, H, CE, 0, N.x-1, 0, N.y-1, N.z-11, N.z-1, psizE.x, psizE.y, bE, aE, 1, 1);

		updateSrc(N, E, tstep);

		updateH(N, E, H);
		updateCPMLx(N, E, H, CE, 1, Npml, 1, N.y, 1, N.z, psixH.y, psixH.z, bH, aH, 0, 0);
		updateCPMLx(N, E, H, CE, N.x-10, N.x, 1, N.y, 1, N.z, psixH.y, psixH.z, bH, aH, 0, 1);
		updateCPMLy(N, E, H, CE, 1, N.x, 1, Npml, 1, N.z, psiyH.z, psiyH.x, bH, aH, 0, 0);
		updateCPMLy(N, E, H, CE, 1, N.x, N.y-10, N.y, 1, N.z, psiyH.z, psiyH.x, bH, aH, 0, 1);
		updateCPMLz(N, E, H, CE, 1, N.x, 1, N.y, 1, Npml, psizH.x, psizH.y, bH, aH, 0, 0);
		updateCPMLz(N, E, H, CE, 1, N.x, 1, N.y, N.z-10, N.z, psizH.x, psizH.y, bH, aH, 0, 1);
