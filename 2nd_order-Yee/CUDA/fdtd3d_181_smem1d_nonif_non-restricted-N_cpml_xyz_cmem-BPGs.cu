...
__host__ void selectBPGs(int Ntot, int TPB, int *NK, dim3 *Dg, int *idx0) {
	int i;
	int BPG = Ntot%TPB == 0 ? Ntot/TPB : Ntot/TPB + 1;
...
__host__ void initMainArrays(int Ntot, P1F3 F) {
	...
	Db = dim3( TPB );
	selectBPGs( Ntot, TPB, NK, Dg, idx0 );
...
int main() {
	...
	int *NK_main, *idx0_main;
	dim3 Db_main, *Dg_main;

	Ntot = N.x*N.y*N.z;
	TPB = selectTPB( Ntot, N.y*N.z );
	Db_main = dim3( TPB );
	selectBPGs( Ntot, TPB, NK_main, Dg_main, idx0_main );
