...
__global__ void updateE(N3 N, P1F3 E, P1F3 H, P1F3 CE) {
	int tk = threadIdx.x;
...
__global__ void updateH(N3 N, P1F3 E, P1F3 H) {
	int tk = threadIdx.x;
	int idx = blockIdx.x*blockDim.x + tk;
	int Nyz = N.y*N.z;
	int eidx = idx + Nyz;
...
__global__ void updateSrc(N3 N, P1F3 E, int tstep) {
	int idx, ijk;
	idx = threadIdx.x;
	//ijk = (idx+1)*N.y*N.z + (N.y/2)*N.z + (N.z/2);
	//ijk = (idx+1)*N.y*N.z + (N.y/2 - 30)*N.z + (N.z/2 - 50);
	//ijk = (N.x/2 - 30)*N.y*N.z + (idx)*N.z + (N.z/2 - 50);
	//ijk = (N.x/2-30)*N.y*N.z + (N.y/2-50)*N.z + idx;
	ijk = (N.x/2-30)*N.y*N.z + (N.y/2-50)*N.z + (N.z/2);

	//E.x[ijk] += sin(0.1*tstep);
	//E.y[ijk] += sin(0.1*tstep);
...
__global__ void updateCPMLyH(N3 N, P1F3 E, P1F3 H, float *psi1, float *psi2, int backward) {
	int pidx = blockIdx.x*blockDim.x + threadIdx.x;
	int i = pidx/(NPML*N.z);
	int pj = ( pidx/N.z )%NPML + 1 + backward*(NPML+1);
...
__global__ void updateCPMLzE(N3 N, P1F3 E, P1F3 H, P1F3 CE, float *psi1, float *psi2, int backward) {
	...
	int idx = pidx + 1 + (pidx/NPMLp)*(N.z-NPMLp) + backward*(N.z-NPMLp-1);
	int eidx = idx + N.y*N.z;
...
__global__ void updateCPMLzH(N3 N, P1F3 E, P1F3 H, float *psi1, float *psi2, int backward) {
	...
	int idx = pidx + (pidx/NPMLp + backward)*(N.z-NPMLp);
	int eidx = idx + N.y*N.z;
...
*/	
	...
	for ( tstep=1; tstep<=TMAX; tstep++) {
		...
		updateE <<<Dg_main,Db_main,Ns_main>>> ( N, devE, devH, devCE );
		/*
		...
		*/	
		...
		updateH <<<Dg_main,Db_main,Ns_main>>> ( N, devE, devH );
		/*
		...
		*/
		if ( tstep/1000*1000 == tstep ) {
			...
			dumpToH5(N.x+2, N.y, N.z, 0, N.y/2, 0, N.x-1, N.y/2, N.z-1, Ex, "gpu_png/Ex-%05d.h5", tstep);
			exec("h5topng -ZM0.1 -y0 -S4 -c /usr/share/h5utils/colormaps/dkbluered gpu_png/Ex-%05d.h5", tstep);
			dumpToH5(N.x+2, N.y, N.z, 0, N.y/2, 0, N.x-1, N.y/2, N.z-1, Ey, "gpu_png/Ey-%05d.h5", tstep);
			exec("h5topng -ZM0.1 -y0 -S4 -c /usr/share/h5utils/colormaps/dkbluered gpu_png/Ey-%05d.h5", tstep);
			dumpToH5(N.x+2, N.y, N.z, 0, N.y/2, 0, N.x-1, N.y/2, N.z-1, Ez, "gpu_png/Ez-%05d.h5", tstep);
			exec("h5topng -ZM0.1 -y0 -S4 -c /usr/share/h5utils/colormaps/dkbluered gpu_png/Ez-%05d.h5", tstep);
			dumpToH5(N.x+2, N.y, N.z, 0, N.y/2, 0, N.x-1, N.y/2, N.z-1, Hx, "gpu_png/Hx-%05d.h5", tstep);
			exec("h5topng -ZM0.1 -y0 -S4 -c /usr/share/h5utils/colormaps/dkbluered gpu_png/Hx-%05d.h5", tstep);
			dumpToH5(N.x+2, N.y, N.z, 0, N.y/2, 0, N.x-1, N.y/2, N.z-1, Hy, "gpu_png/Hy-%05d.h5", tstep);
			exec("h5topng -ZM0.1 -y0 -S4 -c /usr/share/h5utils/colormaps/dkbluered gpu_png/Hy-%05d.h5", tstep);
			dumpToH5(N.x+2, N.y, N.z, 0, N.y/2, 0, N.x-1, N.y/2, N.z-1, Hz, "gpu_png/Hz-%05d.h5", tstep);
			exec("h5topng -ZM0.1 -y0 -S4 -c /usr/share/h5utils/colormaps/dkbluered gpu_png/Hz-%05d.h5", tstep);
	...
	//for ( tstep=1; tstep<=10; tstep++ ) updateE <<<Dg_main,Db_main,Ns_main>>> ( N, devE, devH, devCE );
