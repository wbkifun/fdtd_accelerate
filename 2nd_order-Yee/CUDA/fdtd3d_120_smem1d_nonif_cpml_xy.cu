...
__global__ void updateCPMLzE(
		int Ny, int Nzpit, int Npmlpit, int TPB,
		P1F3 E, P1F3 H, P1F3 CE,
		float *psi1, float *psi2,
		int backward) {
	int pidx = blockIdx.x*TPB + threadIdx.x;
	int i = pidx/Ny*Npmlpit;
	int j = ( pidx - i*Ny*Npmlpit )/Npmlpit;
	int pk = pidx - i*Ny*Npmlpit - j*Npmlpit;

	int idx = pidx + (j+i*Ny)*(Nzpit-Npmlpit);// + backward*(N.z-Npml-1);
	int eidx = idx + Ny*Nzpit;
	...
	if ( pk<Npml ) {
		int pi = pk + backward*Npml;
		psi1[pidx] = rcmbE[pi]*psi1[pidx] + rcmaE[pi]*( hy[pk+1] - hy[pk] );
		E.x[eidx] -= CE.x[idx]*psi1[pidx];

		psi2[pidx] = rcmbE[pi]*psi2[pidx] + rcmaE[pi]*( hx[pk+1] - hx[pk] );
...
__global__ void init_boundary_xE(N3 N, int Nyzpit, P1F3 E) {
	int idx = N.x*Nyzpit + blockIdx.x*blockDim.x + threadIdx.x;
...
__global__ void init_boundary_yE(N3 N, int Nzpit, int Nyzpit, P1F3 E) {
	...
	int i = ti/Nzpit;
	int idx = (i+2)*Nyzpit - (i+1)*Nzpit + ti;
...
__global__ void init_boundary_zE(N3 N, int Nzpit, int Nyzpit, P1F3 E) {
	int ti = blockIdx.x*blockDim.x + threadIdx.x;
	int i = ti/N.y;
	int idx = (N.z-1) + (i+1)*Nyzpit + (ti-i*N.y)*Nzpit;
...
__global__ void init_boundary_zH(N3 N, int Nzpit, int Nyzpit, P1F3 H) {
	int ti = blockIdx.x*blockDim.x + threadIdx.x;
	int i = ti/N.y;
	int idx = i*Nyzpit + (ti-i*N.y)*Nzpit;
...
int main() {
	...
	printf("pitch_pmlz= %u, Nzpml_pitch= %d\n", pitch_pmlz, Nzpml_pitch);
	...
	for ( tstep=1; tstep<=500; tstep++) {
		...
		//updateCPMLzE <<<Dgpmlz,Dbpmlz,Nspmlz>>> ( N.y, Nz_pitch, Nzpml_pitch, TPBpmlz, devE, devH, devCE, psizE.x.f, psizE.y.f, 0); 
