...
__global__ void initArrays(P1F3 E, P1F3 H) {
	int idx;
	idx = blockIdx.x*TPB[th_max] + threadIdx.x;
...
__global__ void updateE(P1F3 E, P1F3 H, P1F3 CE) {
	int tk, idx;
	tk = threadIdx.x;
	idx = blockIdx.x*TPB[th_main] + tk;
	...
	float* hx = (float*) hs;
	float* hy = (float*) &hx[TPB[th_main]+1];
	float* hz = (float*) &hy[TPB[th_main]+1];

	if ( i<DIM[x_axis] && k<DIM[z_axis] ) {
		hx[tk] = H.x[idx];
		hy[tk] = H.y[idx];
		hz[tk] = H.z[idx];
		if ( tk==TPB[th_main]-1 && k<DIM[z_axis]-1 ) {
			hx[tk+1] = H.x[idx+1];
			hy[tk+1] = H.y[idx+1];
		}
	}
...
__global__ void updateSrc(P1F3 E, int tstep) {
	...
	idx = blockIdx.x*TPB[th_src] + threadIdx.x;
...
__global__ void updateH(P1F3 E, P1F3 H) {
	int tk, idx;
	tk = threadIdx.x;
	idx = blockIdx.x*TPB[th_main] + tk;
	...
	float* ex = (float*) es;
	float* ey = (float*) &ex[TPB[th_main]+1];
	float* ez = (float*) &ey[TPB[th_main]+1];
...
int main() {
	...
	for ( tstep=1; tstep<=10; tstep++) {
		// Update on the GPU
		updateE <<<Dg,Db,Ns>>> ( devE, devH, devCE );
		/*
		//updateSrc <<<Dgsrc,Dbsrc>>> ( devE, tstep );
		//updateH <<<Dg,Db,Ns>>> ( devE, devH );
