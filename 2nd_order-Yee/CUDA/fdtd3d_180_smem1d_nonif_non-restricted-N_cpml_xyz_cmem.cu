...
__global__ void updateCPMLzE_cmem(N3 N, P1F3 E, P1F3 H, P1F3 CE, float *psi1, float *psi2, int backward) {
	...
	int idx = pidx + (pidx/NPML)*(N.z - NPML) + backward*(N.z - NPML - 1);
	int eidx = idx + N.y*N.z;
...
__global__ void updateCPMLzH_cmem(N3 N, P1F3 E, P1F3 H, float *psi1, float *psi2, int backward) {
	int tk = threadIdx.x;
	int pidx = blockIdx.x*blockDim.x + tk;
	int pk = pidx%NPML + backward*(NPML+1);
...
int main() {
	...
	*/
	...
	for ( tstep=1; tstep<=TMAX; tstep++) {
		...
		updateCPMLxE_cmem <<<Dg_pmlx,Db_pmlx>>> ( N, devE, devH, devCE, psixE.y.f, psixE.z.f, 0);
		updateCPMLxE_cmem <<<Dg_pmlx,Db_pmlx>>> ( N, devE, devH, devCE, psixE.y.b, psixE.z.b, 1); 
		updateCPMLyE_cmem1 <<<Dg_pmly,Db_pmly>>> ( N, devE, devH, devCE, psiyE.z.f, psiyE.x.f, 0);
		updateCPMLyE_cmem1 <<<Dg_pmly,Db_pmly>>> ( N, devE, devH, devCE, psiyE.z.b, psiyE.x.b, 1);
		*/
		updateCPMLzE_cmem <<<Dg_pmlz,Db_pmlz,Ns_pmlz>>> ( N, devE, devH, devCE, psizE.x.f, psizE.y.f, 0);
		//updateCPMLzE_cmem <<<Dg_pmlz,Db_pmlz,Ns_pmlz>>> ( N, devE, devH, devCE, psizE.x.b, psizE.y.b, 1);
		/*	
		// Source update
		...
		updateCPMLxH_cmem <<<Dg_pmlx,Db_pmlx>>> ( N, devE, devH, psixH.y.f, psixH.z.f, 0); 
		updateCPMLxH_cmem <<<Dg_pmlx,Db_pmlx>>> ( N, devE, devH, psixH.y.b, psixH.z.b, 1); 
		updateCPMLyH_cmem1 <<<Dg_pmlx,Db_pmlx>>> ( N, devE, devH, psiyH.z.f, psiyH.x.f, 0);
		updateCPMLyH_cmem1 <<<Dg_pmlx,Db_pmlx>>> ( N, devE, devH, psiyH.z.b, psiyH.x.b, 1);
		//updateCPMLzH_cmem <<<Dg_pmlz,Db_pmlz,Ns_pmlz>>> ( N, devE, devH, psizH.x.f, psizH.y.f, 0);
		//updateCPMLzH_cmem <<<Dg_pmlz,Db_pmlz,Ns_pmlz>>> ( N, devE, devH, psizH.x.b, psizH.y.b, 1);
		*/
		if ( tstep/50*50 == tstep ) {
			...
			dumpToH5(N.x+2, N.y, N.z, 0, 0, N.z/2, N.x, N.y-1, N.z/2, Ex, "gpu_png/Ex-%05d.h5", tstep);
			exec("h5topng -ZM0.1 -z0 -S4 -c /usr/share/h5utils/colormaps/dkbluered gpu_png/Ex-%05d.h5", tstep);
			dumpToH5(N.x+2, N.y, N.z, 0, 0, N.z/2, N.x, N.y-1, N.z/2, Ey, "gpu_png/Ey-%05d.h5", tstep);
			exec("h5topng -ZM0.1 -z0 -S4 -c /usr/share/h5utils/colormaps/dkbluered gpu_png/Ey-%05d.h5", tstep);
			dumpToH5(N.x+2, N.y, N.z, 0, 0, N.z/2, N.x, N.y-1, N.z/2, Ez, "gpu_png/Ez-%05d.h5", tstep);
			exec("h5topng -ZM0.1 -z0 -S4 -c /usr/share/h5utils/colormaps/dkbluered gpu_png/Ez-%05d.h5", tstep);
			dumpToH5(N.x+2, N.y, N.z, 0, 0, N.z/2, N.x, N.y-1, N.z/2, Hx, "gpu_png/Hx-%05d.h5", tstep);
			exec("h5topng -ZM0.1 -z0 -S4 -c /usr/share/h5utils/colormaps/dkbluered gpu_png/Hx-%05d.h5", tstep);
			dumpToH5(N.x+2, N.y, N.z, 0, 0, N.z/2, N.x, N.y-1, N.z/2, Hy, "gpu_png/Hy-%05d.h5", tstep);
			exec("h5topng -ZM0.1 -z0 -S4 -c /usr/share/h5utils/colormaps/dkbluered gpu_png/Hy-%05d.h5", tstep);
			dumpToH5(N.x+2, N.y, N.z, 0, 0, N.z/2, N.x, N.y-1, N.z/2, Hz, "gpu_png/Hz-%05d.h5", tstep);
