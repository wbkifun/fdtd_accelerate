...
*/
	...
	// time test
	t0 = time(0);
	for ( tstep=1; tstep<=TMAX; tstep++) for ( i=0; i<NK; i++) updateE <<<Dg_main[i],Db_main,Ns_main>>> ( N, devE, devH, devCE, idx0[i] );
	updateTimer(t0, tstep, time_str);
	printf("main E: \t%s\n", time_str);

	t0 = time(0);
	for ( tstep=1; tstep<=TMAX; tstep++) for ( i=0; i<NK; i++) updateH <<<Dg_main[i],Db_main,Ns_main>>> ( N, devE, devH, idx0[i] );
	updateTimer(t0, tstep, time_str);
	printf("main H: \t%s\n", time_str);
