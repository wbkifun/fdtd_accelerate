...
void updateE(N3 N, P3F3 E, P3F3 H, P3F3 CE) {
	...
	shared(N, E, H, CE) private(e, ce, h1, h2, h3, h4, i, j, k) \
	schedule(static)
	for (i=0;i<N.x-1;i++){
		for (j=0;j<N.y-1;j++){
			for (k=0;k<N.z;k+=4){
	...
	i=N.x-1;
	#pragma omp parallel for \
	shared(N, E, H, CE) private(e, ce, h1, h2, h3, h4, j, k) \
	schedule(static)
	for (j=0;j<N.y-1;j++){
		for (k=0;k<N.z;k+=4){
			e = LOAD(&E.x[i][j][k]);
	...
	j=N.y-1;
	#pragma omp parallel for \
	shared(N, E, H, CE) private(e, ce, h1, h2, h3, h4, i, k) \
	schedule(static)
	for (i=0;i<N.x-1;i++){
		for (k=0;k<N.z;k+=4){
			e = LOAD(&E.y[i][j][k]);
...
void updateH(N3 N, P3F3 E, P3F3 H) {
	int i,j,k;
	v4sf h, ch={0.5, 0.5, 0.5, 0.5}, e1, e2, e3, e4; 

	//omp_set_num_threads(8);
	#pragma omp parallel for \
	shared(N, E, H) private(h, ch, e1, e2, e3, e4, i, j, k) \
	schedule(static)
	for (i=1;i<N.x;i++){
		for (j=1;j<N.y;j++){
			for (k=0;k<N.z;k+=4){
