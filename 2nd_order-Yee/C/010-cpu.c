...
void updateH(N3 N, P3F3 E, P3F3 H, P3F3 CH) {
	...
	for (i=1; i<N.x; i++) {
		for (j=1; j<N.y; j++) {
			for (k=1; k<N.z; k++) {
				H.x[i][j][k] -= CH.x[i][j][k]*(E.z[i][j][k] - E.z[i][j-1][k] - E.y[i][j][k] + E.y[i][j][k-1]);
				H.y[i][j][k] -= CH.y[i][j][k]*(E.x[i][j][k] - E.x[i][j][k-1] - E.z[i][j][k] + E.z[i-1][j][k]);
				H.z[i][j][k] -= CH.z[i][j][k]*(E.y[i][j][k] - E.y[i-1][j][k] - E.x[i][j][k] + E.x[i][j-1][k]);
			}
		}
	}
