...
class fdtd1d:
	...
	def update_src_e(s, tstep, spt, dt=1./2, **kargs):
		if kargs['ft'] == 'sin':
			s.ez[spt] += np.sin(np.pi * kargs['frequency'] * tstep)

		elif kargs['ft'] == 'gaussian':
			s.ez[spt] += np.exp(- 0.5 * ( np.float64(tstep - kargs['t0']) * dt/ kargs['sigma'] )**2 )
...
# save aux fields
aux_ft = np.zeros(tmax, dtype=np.float64)
aux_hy1 = np.zeros(tmax, dtype=np.float64)
aux_hy2 = np.zeros_like(aux_hy1)
aux_ez1 = np.zeros_like(aux_hy1)
aux_ez2 = np.zeros_like(aux_hy1)

# main loop
for tstep in xrange(tmax):
	fdtd.update_e()

	aux_hy1[tstep] = fdtd_aux.hy[2]
	aux_hy2[tstep] = fdtd_aux.hy[-2]
	fdtd.update_tfsf_e(spt1, spt2, fdtd_aux)
	fdtd_aux.update_e()
	#fdtd_aux.update_src_e(tstep, 1, ft='sin', frequency=1./50)
	fdtd_aux.update_src_e(tstep, 1, ft='gaussian', t0=200, sigma=20)
	aux_ft[tstep] = fdtd_aux.ez[1]

	fdtd.update_h()

	aux_ez1[tstep] = fdtd_aux.ez[2]
	aux_ez2[tstep] = fdtd_aux.ez[-2]
...
print ''
np.save('./tfsf_ft_ez.npy', aux_ft)
np.save('./tfsf_aux_hy1.npy', aux_hy1)
np.save('./tfsf_aux_hy2.npy', aux_hy2)
np.save('./tfsf_aux_ez1.npy', aux_ez1)
np.save('./tfsf_aux_ez2.npy', aux_ez2)
