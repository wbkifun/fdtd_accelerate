...
# prepare pml
#s1 = np.ones(npml, 'f') * 0.4
#s2 = 0.2		# (sigma * dt) / 2
s1 = ( (np.linspace(1,npml,npml) / npml) ** 6 ) * 1.	# sigma * dt	
s2 = ( (np.linspace(1,npml,npml) / npml) ** 6 ) * 0.5	# (sigma * dt) / 2	
pca1 = (2. - s1[:]) / (2 + s1[:])	# pml coefficients
pcb1 = 1. / (2 + s1[:])
pca2 = (1. - np.tanh(s2[:])) / (1. + np.tanh(s2[:])) 
pcb2 = 0.5 / ((1. + np.tanh(s2[:])) * np.cosh(s2[:])) 

# main loop
for tstep in xrange(1, tmax+1):
	#pulse = np.sin(2*np.pi*frequency*dt*tstep)
	pulse = np.exp( -0.5*( float(tstep-70)/10 )**2 )
	...
	ez2[-npml-1:-1] = pca2[:] * ez2[-npml-1:-1] + np.exp(s2) * pcb2[:] * (hy2[-npml:] - hy2[-npml-1:-1])
	ez2[120] += pulse
	hy2[1:-npml] -= 0.5 * (- ez2[1:-npml] + ez2[:-npml-1])
	hy2[-npml:] = pca2[:] * hy2[-npml:] - np.exp(-s2) * pcb2[:] * (- ez2[-npml:] + ez2[-npml-1:-1]) 


	if tstep%tgap == 0:
		print "tstep= %d\r" % (tstep),
		sys.stdout.flush()
		line1.set_ydata(ez1[:]*10)
		line2.set_ydata(ez2[:]*10)
		plt.draw()
