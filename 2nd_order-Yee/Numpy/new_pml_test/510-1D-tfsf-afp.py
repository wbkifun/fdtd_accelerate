...
class fdtd1d:
	...
	def update_src_e(s, tstep, spt, **kargs):
		if kargs['ft'] == 'sin':
			...
			s.ez[spt] += np.exp(- 0.5 * ( np.float32(tstep - kargs['t0']) / kargs['sigma'] )**2 )


	def update_all(s, tstep, **kargs):
		s.update_e()
		s.update_abc_e()
		s.update_src_e(tstep, 1, **kargs)
		s.update_h()
		s.update_abc_h()
...
for tstep in xrange(tmax):
	...
	fdtd_aux.update_all(tstep, ft='sin', frequency=1./50)
	#fdtd_aux.update_all(tstep, ft='gaussian', t0=150, sigma=20)
