...
sigma_dt = np.ones(npml+1, 'f')*(0.01)
pml_ez = np.zeros(npml+1, 'f')
pml_hy = np.zeros(npml+1, 'f')

# main loop
for tstep in xrange(1, tmax+1):
	pml_ez[:] -= (1 + sigma_dt)*ez[-npml-2:-1]
	ez[:-npml-1] += 0.5*(hy[1:-npml] - hy[:-npml-1])
	ez[-npml-1:-1] += 0.5*(pml_hy[1:] - pml_hy[:-1])
	pml_ez[:] += (1 - sigma_dt)*ez[-npml-2:-1]
	...
	pml_hy[:] -= (1 + sigma_dt)*hy[-npml-1:]
	hy[1:-npml] -= 0.5*(- ez[1:-npml] + ez[:-npml-1])
	hy[-npml:] -= 0.5*(- pml_ez[1:] + pml_ez[:-1])
	pml_hy[:] += (1 - sigma_dt)*hy[-npml-1:]
