...
class Fdtd3dGpu:
	...
	def allocations(s):
		...
		s.offsets = []
		s.recvs = {'x':True, 'y':False, 'z':False}
		s.sends = {'x':True, 'y':False, 'z':False}
		s.shapes = {'x':(s.ny,s.nz), 'y':(s.nx,s.nz), 'z':(s.nx,s.ny)}
		for queue in s.queues:
			for mapbuf in s.mapbufs:
				pinbuf = cl.Buffer(s.context, mf.ALLOC_HOST_PTR, 2*s.ny*s.nz*np.nbytes['float32'])
				s.mapbuf[0] = cl.enqueue_map_buffer(queue, pinbuf, cl.map_flags.WRITE, 0, (2,ny,nz), np.float32, 'C')

		for nx in s.nxs:
			pinbuf = cl.Buffer(s.context, mf.ALLOC_HOST_PTR, 2*nyz_nbytes)
			mapbuf = cl.enqueue_map_buffer(
