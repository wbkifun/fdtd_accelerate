...
kernels ="""
__global__ void update_h(double *ex, double *ey, double *ez, double *hx, double *hy, double *hz) {
	int tx = threadIdx.x;
	int idx = Dx*blockIdx.x + tx;

	__shared__ double s[3*Dx+2];
	double *sz = s;
	double *sy = &sz[Dx+1];
	double *sx = &sy[Dx+1];
...
__global__ void update_e(double *ex, double *ey, double *ez, double *hx, double *hy, double *hz, double *cex, double *cey, double *cez) {
	int tx = threadIdx.x;
	int idx = Dx*blockIdx.x + tx;
