...
kernels ="""
__global__ void update_h(int idx0, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz) {
	int tx = threadIdx.x;
	int idx = TPB*blockIdx.x + tx + idx0;
	int i = idx/(nyz);
	int j = (idx - i*nyz)/nz;
	int k = idx%nz;
...
__global__ void update_e(int idx0, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *cex, float *cey, float *cez) {
	int tx = threadIdx.x;
	int idx = TPB*blockIdx.x + tx + idx0;
	int i = idx/(nyz);
	int j = (idx - i*nyz)/nz;
...
if __name__ == '__main__':
	...
	update_h.prepare("iPPPPPP", block=(tpb,1,1))
	update_e.prepare("iPPPPPPPPP", block=(tpb,1,1))
	...
	for tn in xrange(1, 300+1):
		for i in xrange(ng): update_h.prepared_call(
				(bpg,1), np.int32(i*bpg*tpb), ex_gpu, ey_gpu, ez_gpu, hx_gpu, hy_gpu, hz_gpu)

		for i in xrange(ng): update_e.prepared_call(
				(bpg,1), np.int32(i*bpg*tpb), ex_gpu, ey_gpu, ez_gpu, hx_gpu, hy_gpu, hz_gpu, 
