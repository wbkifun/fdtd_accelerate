...
# prepare kernels
mod1 = SourceModule( kernels1.replace('TPB',str(tpb)).replace('nyz',str(ny*nz)).replace('nx',str(nx)).replace('ny',str(ny)).replace('nz',str(nz)), options=['-Xptxas','-dlcm=ca'] )
...
measure_exec_time('tex3D (64,16,1)', e5.prepared_call, (nz/Dx,nx*ny/Dy), *eh_args)
