...
kernels ="""
__global__ void update_h(float *hx, float *hy, float *hz, float *ex, float *ey, float *ez) {
	int tx = threadIdx.x;
...
__global__ void update_e(float *hx, float *hy, float *hz, float *ex, float *ey, float *ez, float *cex, float *cey, float *cez) {
	int tx = threadIdx.x;
	int idx = TPB*blockIdx.x + tx;
	int i = idx/(nyz);
	int j = (idx - i*nyz)/nz;
...
if __name__ == '__main__':
	...
	for tn in xrange(1, tmax+1):
		update_h.prepared_call(
				(bpg,1), hx_gpu, hy_gpu, hz_gpu, ex_gpu, ey_gpu, ez_gpu)
