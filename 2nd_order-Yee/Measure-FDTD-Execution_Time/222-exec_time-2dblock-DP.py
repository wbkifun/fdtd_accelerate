...
kernels ="""
__global__ void update_h(int by0, double *ex, double *ey, double *ez, double *hx, double *hy, double *hz) {
	int tx = threadIdx.x;
	int ty = threadIdx.y;
	int bx = blockIdx.x;
	...
	__shared__ double sx[Dy+1][Dx+1];
	__shared__ double sy[Dy][Dx+1];
	__shared__ double sz[Dy+1][Dx];
...
__global__ void update_e(int by0, double *ex, double *ey, double *ez, double *hx, double *hy, double *hz, double *cex, double *cey, double *cez) {
	int tx = threadIdx.x;
	int ty = threadIdx.y;
	int bx = blockIdx.x;
	int by = blockIdx.y + by0;
...
__global__ void update_src(double tn, double *f) {
	int idx = threadIdx.x;
...
print '(%d, %d, %d) double precision' % (nx, ny, nz),
...
cey_gpu = cuda.to_device( set_c(f,'yz') )
cez_gpu = cuda.to_device( set_c(f,'yz') )
...
update_e.prepare("iPPPPPPPPP", block=(Dx,Dy,1))
...
for tn in xrange(1, tmax+1):
	...
	for i, bpg in enumerate(bpg_list): update_e.prepared_call(bpg, np.int32(i*MBy), *ehc_args)
	stop2.record()
	stop2.synchronize()
	exec_time['update_e'][tn-1] = stop2.time_since(start2)
	start2.record()

	update_src.prepared_call((1,1), np.float64(tn), ez_gpu)
	stop2.record()
	stop2.synchronize()
