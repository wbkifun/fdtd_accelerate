...
__global__ void update_e(float *ex, float *ey, float *ez, float *hx, float *hy, float *hz) {
	...
	ex[idx] += tex3D(tcex,i,j,k)*( sz[ty+1][tx] - sz[ty][tx] - sy[ty][tx+1] + sy[ty][tx] );
	if( i<nx-1 ) {
		ey[idx] += tex3D(tcey,i,j,k)*( sx[ty][tx+1] - sx[ty][tx] - hz[idx+nyz] + sz[ty][tx] );
		ez[idx] += tex3D(tcez,i,j,k)*( hy[idx+nyz] - sy[ty][tx] - sx[ty+1][tx] + sx[ty][tx] );
	}
}
...
if __name__ == '__main__':
	...
	mcopy.depth = nx
	arrcopy(mcopy, set_c(f,'yz'), tcex_gpu)
	arrcopy(mcopy, set_c(f,'zx'), tcey_gpu)
	arrcopy(mcopy, set_c(f,'xy'), tcez_gpu)
	...
	update_e.prepare("PPPPPP", block=(16,16,1), texrefs=[tcex,tcey,tcez])
