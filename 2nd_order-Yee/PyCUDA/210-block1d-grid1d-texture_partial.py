...
__global__ void update_e(int nx, int ny, int nz, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz) {
	...
	if( idx < (nx*ny-1)*nz ) 
		ex[idx] += tex1Dfetch(tcex,idx)*( hz[idx+nz] - hz[idx] - hy[idx+1] + hy[idx] );

	if( idx < nx*ny*(nz-1) ) {
		ey[idx] += tex1Dfetch(tcey,idx)*( hx[idx+1] - hx[idx] - hz[idx+ny*nz] + hz[idx] );
		ez[idx] += tex1Dfetch(tcez,idx)*( hy[idx+ny*nz] - hy[idx] - hx[idx+nz] + hx[idx] );
	}
}
...
__global__ void update_h(int nx, int ny, int nz, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz) {
	...
	if( idx < nx*ny*nz ) {
		if( idx > nz ) 
			hx[idx] -= tex1Dfetch(tchx,idx)*( ez[idx] - ez[idx-nz] - ey[idx] + ey[idx-1] );

		if( idx > ny*nz ) {
			hy[idx] -= tex1Dfetch(tchy,idx)*( ex[idx] - ex[idx-1] - ez[idx] + ez[idx-ny*nz] );
			hz[idx] -= tex1Dfetch(tchz,idx)*( ey[idx] - ey[idx-ny*nz] - ex[idx] + ex[idx-nz] );
		}
	}
}
...
if __name__ == '__main__':
	...
	#tcez.set_format(cuda.array_format.FLOAT, 1)
	print tcez.get_format()
	print tcez.get_flags()
	print tcez.get_address_mode(2)
	print tcez.get_filter_mode()
	...
	update_e.prepare("iiiPPPPPP", block=Db, texrefs=[tcex,tcey,tcez])
	update_h.prepare("iiiPPPPPP", block=Db, texrefs=[tchx,tchy,tchz])
