...
__global__ void update_e(int nx, int ny, int nz, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz) {
	...
	if( idx < nx*ny*(nz-1) ) {
		...
		ey[idx] += tex1Dfetch(tcey,idx)*( hx[idx+nx*ny] - sx[tx] - hz_x + sz[tx] );
	}

	if( idx < nx*(ny*nz-1) )
		ez[idx] += tex1Dfetch(tcez,idx)*( hy_x - sy[tx] - hx[idx+nx] + sx[tx] );
}
...
__global__ void update_h(int nx, int ny, int nz, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz) {
	...
	if( idx > nx*ny ) {
		hx[idx] -= tex1Dfetch(tchx,idx)*( sz[tx] - ez[idx-nx] - sy[tx] + ey[idx-nx*ny] );
		hy[idx] -= tex1Dfetch(tchy,idx)*( sx[tx] - ex[idx-nx*ny] - sz[tx] + ez_x );
	}
	
	if( idx > nx ) 
		hz[idx] -= tex1Dfetch(tchz,idx)*( sy[tx] - ey_x - sx[tx] + ex[idx-nx] );
}
