...
__global__ void update_e(float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *cex, float *cey, float *cez) {
	...
	__shared__ float sy[Dy][2*Dx];
	__shared__ float sz[Dy+1][2*Dx];
	...
	for( int bx=0; bx<nx/Dx; bx++ ) {
		...
		if( bx == 0 ) {
			...
			sy[ty][tx] = sy[ty][tx+Dx];
			sz[ty][tx] = sz[ty][tx+Dx];
		}
		__syncthreads();
		if( bx < nx/Dx-1 ) {
			sy[ty][tx+Dx] = hy[idx+Dx];
			sz[ty][tx+Dx] = hz[idx+Dx];
		}
		if( ty==Dy-1 && j<ny-1 ) {
			sx[Dx][tx] = hx[idx+nx];
			sz[Dx][tx] = hz[idx+nx];
		...
		if( k<nz-1 ) {
			ex[idx] += cex[idx]*( sz[ty+1][tx] - sz[ty][tx] - hy[idx+nxy] + sy[ty][tx] );
			ey[idx] += cey[idx]*( hx[idx+nxy] - sx[ty][tx] - sz[ty][tx+1] + sz[ty][tx] );
		}
