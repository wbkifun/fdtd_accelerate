...
__global__ void update_h(float *ex, float *ey, float *ez, float *hx, float *hy, float *hz) {
	...
	float tmp_hzx, tmp_hy;

	sx[tx] = ex[idx];
	if( tx == 0 ) sx[tx-1] = ex[idx-1];
	__syncthreads();
	tmp_hzx = sx[tx] - ex[idx-nz];
	tmp_hy = sx[tx] - sx[tx-1];

	sy[tx] = ey[idx];
	if( tx == 0 ) sy[tx-1] = ey[idx-1];
	__syncthreads();
	if( i>0 && j>0 ) hz[idx] -= 0.5*( sy[tx] - ey[idx-nyz] - tmp_hzx );
	tmp_hzx = sy[tx] - sy[tx-1];

	sz[tx] = ez[idx];
	__syncthreads();
	if( i>0 && k>0 ) hy[idx] -= 0.5*( tmp_hy - sz[tx] + ez[idx-nyz] );
	if( j>0 && k>0 ) hx[idx] -= 0.5*( sz[tx] - ez[idx-nz] - tmp_hzx );
}
...
__global__ void update_e(float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *cex, float *cey, float *cez) {
	...
	sx[tx] = hx[idx];
	if( tx == TPB-1 ) sx[tx+1] = hx[idx+1];
	__syncthreads();
	tmp_ezx = hx[idx+nz] - sx[tx];
	tmp_ey = sx[tx+1] - sx[tx];

	sy[tx] = hy[idx];
	if( tx == TPB-1 ) sy[tx+1] = hy[idx+1];
	__syncthreads();
	if( i<nx-1 && j<ny-1 ) ez[idx] += cez[idx]*( hy[idx+nyz] - sy[tx] - tmp_ezx );
	tmp_ezx = sy[tx+1] - sy[tx];

	sz[tx] = hz[idx];
	__syncthreads();
	if( i<nx-1 && k<nz-1 ) ey[idx] += cey[idx]*( tmp_ey - hz[idx+nyz] + sz[tx] );
	if( j<ny-1 && k<nz-1 ) ex[idx] += cex[idx]*( hz[idx+nz] - sz[tx] - tmp_ezx );
}
