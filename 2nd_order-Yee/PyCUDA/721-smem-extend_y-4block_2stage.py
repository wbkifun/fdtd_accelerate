...
__global__ void update_e(float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *cex, float *cey, float *cez) {
	...
	__syncthreads();
	if( idx<(nx-1)*nyz ) 
		ez[idx+TPB] += cez[idx+TPB]*( hy[idx+nyz+TPB] - s[tx+2*TPB] - tmp_ezx );
	tmp_ezx = s[tx+2*TPB+1] - s[tx+2*TPB];

	__syncthreads();
	s[tx+2*TPB] = hz[idx+2*TPB];
	if( idx<(nx-1)*nyz ) 
		ey[idx+TPB] += cey[idx+TPB]*( tmp_ey - hz[idx+nyz+TPB] + s[tx] );
	__syncthreads();
	ex[idx+TPB] += cex[idx+TPB]*( s[tx+2*TPB] - s[tx] - tmp_ezx );
}
