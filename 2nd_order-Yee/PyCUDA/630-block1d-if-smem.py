...
__global__ void update_h(int nx, int ny, int nz, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *chx, float *chy, float *chz) {
	...
	if( j>0 && k>0 ) hx[idx] -= chx[idx]*( sz[tx] - ez[idx-nx] - sy[tx] + ey[idx-nx*ny] );
	if( i>0 && k>0 ) hy[idx] -= chy[idx]*( sx[tx] - ex[idx-nx*ny] - sz[tx] + sz[tx-1] );
	if( i>0 && j>0 ) hz[idx] -= chz[idx]*( sy[tx] - sy[tx-1] - sx[tx] + ex[idx-nx] );
}
...
if __name__ == '__main__':
	...
	update_h.prepare("iiiPPPPPPPPP", block=Db, shared=(tpb*3+2)*4)
