...
__global__ void update_e(float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *cex, float *cey, float *cez) {
	...
	if( i<nx-1 && k<nz-1 ) ey[idx] += cey[idx]*( s[tx+1] - s[tx] - hz[idx+nyz] + s[tx+4*TPB] );
	if( j<ny-1 && k<nz-1 ) ex[idx] += cex[idx]*( s[tx+5*TPB] - s[tx+4*TPB] - s[tx+2*TPB+1] + s[tx+2*TPB] );
	...
	if( i<nx-1 && k<nz-1 ) ey[idx+2*TPB] += cey[idx+2*TPB]*( s[tx+1] - s[tx] - hz[idx+2*TPB+nyz] + s[tx+4*TPB] );
	if( j<ny-1 && k<nz-1 ) ex[idx+2*TPB] += cex[idx+2*TPB]*( s[tx+5*TPB] - s[tx+4*TPB] - s[tx+2*TPB+1] + s[tx+2*TPB] );
}
