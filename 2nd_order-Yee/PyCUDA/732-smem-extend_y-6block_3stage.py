...
__global__ void update_e(float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *cex, float *cey, float *cez) {
	...
	if( j<ny-1 && k<nz-1 ) ex[idx] += cex[idx]*( s[tx+5*TPB] - s[tx+4*TPB] - tmp_ezx );
	...
	s[tx] = hx[idx+2*TPB];
	__syncthreads();
	tmp_ezx = s[tx] - s[tx+TPB];
	if( tx < TPB-1 ) tmp_ey = s[tx+TPB+1] - s[tx+TPB];
	else tmp_ey = s[0] - s[tx+TPB];

	s[tx+2*TPB] = hy[idx+2*TPB];
	__syncthreads();
	if( i<nx-1 && j<ny-1 ) ez[idx+TPB] += cez[idx+TPB]*( hy[idx+TPB+nyz] - s[tx+3*TPB] - tmp_ezx );
	if( tx < TPB-1 ) tmp_ezx = s[tx+3*TPB+1] - s[tx+3*TPB];
	else tmp_ezx = s[2*TPB] - s[tx+3*TPB];

	s[tx+4*TPB] = hz[idx+2*TPB];
	__syncthreads();
	if( i<nx-1 && k<nz-1 ) ey[idx+TPB] += cey[idx+TPB]*( tmp_ey - hz[idx+TPB+nyz] + s[tx+5*TPB] );
	if( j<ny-1 && k<nz-1 ) ex[idx+TPB] += cex[idx+TPB]*( s[tx+4*TPB] - s[tx+5*TPB] - tmp_ezx );
	...
	__syncthreads();
	if( i<nx-1 && j<ny-1 ) ez[idx+2*TPB] += cez[idx+2*TPB]*( hy[idx+2*TPB+nyz] - s[tx+2*TPB] - tmp_ezx );
	tmp_ezx = s[tx+2*TPB+1] - s[tx+2*TPB];

	s[tx+5*TPB] = hz[idx+3*TPB];
	__syncthreads();
	if( i<nx-1 && k<nz-1 ) ey[idx+2*TPB] += cey[idx+2*TPB]*( tmp_ey - hz[idx+2*TPB+nyz] + s[tx+4*TPB] );
	if( j<ny-1 && k<nz-1 ) ex[idx+2*TPB] += cex[idx+2*TPB]*( s[tx+5*TPB] - s[tx+4*TPB] - tmp_ezx );
}
