...
__global__ void update_e(int nx, int ny, int nz, float *ex, float *ey, float *ez) {
	...
	float szp = sz[tx+1];
	if( tx == blockDim.x - 1 ) {
		syp = tex1Dfetch(thy,idx+1);
		szp = tex1Dfetch(thz,idx+1);
	}
	...
	if( idx < nx*ny*(nz-1) ) {
		ey[idx] += tex1Dfetch(tcey,idx)*( tex1Dfetch(thx,idx+nx*ny) - sx[tx] - szp + sz[tx] );
		ez[idx] += tex1Dfetch(tcez,idx)*( syp - sy[tx] - tex1Dfetch(thx,idx+nx) + sx[tx] );
	}
}
...
__global__ void update_h(int nx, int ny, int nz, float *hx, float *hy, float *hz) {
	...
	float szp = sz[tx-1];
	if( tx == 0 ) {
		syp = tex1Dfetch(tey,idx-1);
		szp = tex1Dfetch(tez,idx-1);
	}

	if( idx < nx*ny*nz ) {
		if( idx > nz ) 
			hx[idx] -= tex1Dfetch(tchx,idx)*( sz[tx] - tex1Dfetch(tez,idx-nx) - sy[tx] + tex1Dfetch(tey,idx-ny*nz) );

		if( idx > ny*nz ) {
			hy[idx] -= tex1Dfetch(tchy,idx)*( sx[tx] - tex1Dfetch(tex,idx-ny*nz) - sz[tx] + szp );
			hz[idx] -= tex1Dfetch(tchz,idx)*( sy[tx] - syp - sx[tx] + tex1Dfetch(tex,idx-nx) );
		}
	}
}
...
if __name__ == '__main__':
	...
	update_e.prepare("iiiPPP", block=Db, texrefs=[thx,thy,thz,tcex,tcey,tcez], shared=tpb*3*4)
	update_h.prepare("iiiPPP", block=Db, texrefs=[tex,tey,tez,tchx,tchy,tchz], shared=tpb*3*4)
