...
__global__ void update_e(int nx, int ny, int nz, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz) {
	...
	if( idx < nx*ny*(nz-1) ) {
		ex[idx] += tex1Dfetch(tcex,idx)*( hz[idx+nx] - hz[idx] - hy[idx+nx*ny] + hy[idx] );
		ey[idx] += tex1Dfetch(tcey,idx)*( hx[idx+nx*ny] - hx[idx] - hz[idx+1] + hz[idx] );
	}

	if( idx < nx*(ny*nz-1) )
		ez[idx] += tex1Dfetch(tcez,idx)*( hy[idx+1] - hy[idx] - hx[idx+nx] + hx[idx] );
}
...
__global__ void update_h(int nx, int ny, int nz, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz) {
	...
	if( idx > nx*ny ) {
		hx[idx] -= tex1Dfetch(tchx,idx)*( ez[idx] - ez[idx-nx] - ey[idx] + ey[idx-nx*ny] );
		hy[idx] -= tex1Dfetch(tchy,idx)*( ex[idx] - ex[idx-nx*ny] - ez[idx] + ez[idx-1] );
	}
	
	if( idx > nx ) 
		hz[idx] -= tex1Dfetch(tchz,idx)*( ey[idx] - ey[idx-1] - ex[idx] + ex[idx-nx] );
}
