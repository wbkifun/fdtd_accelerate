...
__global__ void update_e(float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *cex, float *cey, float *cez) {
	...
	__syncthreads();
	if( i<nx-1 && j<ny-1 ) {
		ez[idx] += cez[idx]*( hy[idx+nyz] - s[tx+5*TPB] - tmp_ezx[0] );
		ez[idx+TPB] += cez[idx+TPB]*( hy[idx+TPB+nyz] - s[tx+6*TPB] - tmp_ezx[1] );
		ez[idx+2*TPB] += cez[idx+2*TPB]*( hy[idx+2*TPB+nyz] - s[tx+7*TPB] - tmp_ezx[2] );
		ez[idx+3*TPB] += cez[idx+3*TPB]*( hy[idx+3*TPB+nyz] - s[tx+8*TPB] - tmp_ezx[3] );
	}
	...
	__syncthreads();
	if( i<nx-1 && k<nz-1 ) {
		ey[idx] += cey[idx]*( tmp_ey[0] - hz[idx+nyz] + s[tx+10*TPB] );
		ey[idx+TPB] += cey[idx+TPB]*( tmp_ey[1] - hz[idx+TPB+nyz] + s[tx+11*TPB] );
		ey[idx+2*TPB] += cey[idx+2*TPB]*( tmp_ey[2] - hz[idx+2*TPB+nyz] + s[tx+12*TPB] );
		ey[idx+3*TPB] += cey[idx+3*TPB]*( tmp_ey[3] - hz[idx+3*TPB+nyz] + s[tx+13*TPB] );
	}
	if( j<ny-1 && k<nz-1 ) {
		ex[idx] += cex[idx]*( s[tx+11*TPB] - s[tx+10*TPB] - tmp_ezx[0] );
		ex[idx+TPB] += cex[idx+TPB]*( s[tx+12*TPB] - s[tx+11*TPB] - tmp_ezx[1] );
		ex[idx+2*TPB] += cex[idx+2*TPB]*( s[tx+13*TPB] - s[tx+12*TPB] - tmp_ezx[2] );
		ex[idx+3*TPB] += cex[idx+3*TPB]*( s[tx+14*TPB] - s[tx+13*TPB] - tmp_ezx[3] );
	}
}
