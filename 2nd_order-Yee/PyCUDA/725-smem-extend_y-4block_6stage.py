...
__global__ void update_e(float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *cex, float *cey, float *cez) {
	...
	__syncthreads();
	if( i<nx-1 && j<ny-1 ) ez[idx+4*TPB] += cez[idx+4*TPB]*( hy[idx+nyz+4*TPB] - s[tx+2*TPB] - tmp_ezx );
	tmp_ezx = s[tx+2*TPB+1] - s[tx+2*TPB];

	__syncthreads();
	s[tx+2*TPB] = hz[idx+5*TPB];
	if( i<nx-1 && k<nz-1 ) ey[idx+4*TPB] += cey[idx+4*TPB]*( tmp_ey - hz[idx+nyz+4*TPB] + s[tx] );
	__syncthreads();
	if( j<ny-1 && k<nz-1 ) ex[idx+4*TPB] += cex[idx+4*TPB]*( s[tx+2*TPB] - s[tx] - tmp_ezx );
	...
	s[tx+2*TPB] = s[tx+3*TPB];
	__syncthreads();
	s[tx+3*TPB] = hy[idx+6*TPB];
	__syncthreads();
	if( i<nx-1 && j<ny-1 ) ez[idx+5*TPB] += cez[idx+5*TPB]*( hy[idx+nyz+5*TPB] - s[tx+2*TPB] - tmp_ezx );
	tmp_ezx = s[tx+2*TPB+1] - s[tx+2*TPB];

	__syncthreads();
	s[tx+2*TPB] = hz[idx+6*TPB];
	if( i<nx-1 && k<nz-1 ) ey[idx+5*TPB] += cey[idx+5*TPB]*( tmp_ey - hz[idx+nyz+5*TPB] + s[tx] );
	__syncthreads();
	if( j<ny-1 && k<nz-1 ) ex[idx+5*TPB] += cex[idx+5*TPB]*( s[tx+2*TPB] - s[tx] - tmp_ezx );
}
...
if __name__ == '__main__':
	...
	for tn in xrange(1, 1000+1):
		...
		update_e.prepared_call(
				(bpg/5,1), ex_gpu, ey_gpu, ez_gpu, hx_gpu, hy_gpu, hz_gpu, 
