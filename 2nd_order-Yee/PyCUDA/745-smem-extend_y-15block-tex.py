...
__global__ void update_e(float *ex, float *ey, float *ez) {
	...
	__syncthreads();
	if( i<nx-1 && j<ny-1 ) {
		ez[idx] += tex1Dfetch(tcez,idx)*( tex1Dfetch(thy,idx+nyz) - s[tx+5*TPB] - tmp_ezx[0] );
		ez[idx+TPB] += tex1Dfetch(tcez,idx+TPB)*( tex1Dfetch(thy,idx+TPB+nyz) - s[tx+6*TPB] - tmp_ezx[1] );
		ez[idx+2*TPB] += tex1Dfetch(tcez,idx+2*TPB)*( tex1Dfetch(thy,idx+2*TPB+nyz) - s[tx+7*TPB] - tmp_ezx[2] );
		ez[idx+3*TPB] += tex1Dfetch(tcez,idx+3*TPB)*( tex1Dfetch(thy,idx+3*TPB+nyz) - s[tx+8*TPB] - tmp_ezx[3] );
	}
	...
	__syncthreads();
	if( i<nx-1 && k<nz-1 ) {
		ey[idx] += tex1Dfetch(tcey,idx)*( tmp_ey[0] - tex1Dfetch(thz,idx+nyz) + s[tx+10*TPB] );
		ey[idx+TPB] += tex1Dfetch(tcey,idx+TPB)*( tmp_ey[1] - tex1Dfetch(thz,idx+TPB+nyz) + s[tx+11*TPB] );
		ey[idx+2*TPB] += tex1Dfetch(tcey,idx+2*TPB)*( tmp_ey[2] - tex1Dfetch(thz,idx+2*TPB+nyz) + s[tx+12*TPB] );
		ey[idx+3*TPB] += tex1Dfetch(tcey,idx+3*TPB)*( tmp_ey[3] - tex1Dfetch(thz,idx+3*TPB+nyz) + s[tx+13*TPB] );
	}
	if( j<ny-1 && k<nz-1 ) {
		ex[idx] += tex1Dfetch(tcex,idx)*( s[tx+11*TPB] - s[tx+10*TPB] - tmp_ezx[0] );
		ex[idx+TPB] += tex1Dfetch(tcex,idx+TPB)*( s[tx+12*TPB] - s[tx+11*TPB] - tmp_ezx[1] );
		ex[idx+2*TPB] += tex1Dfetch(tcex,idx+2*TPB)*( s[tx+13*TPB] - s[tx+12*TPB] - tmp_ezx[2] );
		ex[idx+3*TPB] += tex1Dfetch(tcex,idx+3*TPB)*( s[tx+14*TPB] - s[tx+13*TPB] - tmp_ezx[3] );
	}
}
