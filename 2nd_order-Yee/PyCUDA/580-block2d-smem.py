...
__global__ void update_e(int nx, int ny, int nz, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *cex, float *cey, float *cez) {
	...
	if( idx < nx*ny*(nz-1) ) {
		ex[idx] += cex[idx]*( hz_y - sz[sidx] - hy[idx+nx*ny] + sy[sidx] );
		ey[idx] += cey[idx]*( hx[idx+nx*ny] - sx[sidx] - hz_x + sz[sidx] );
	}
...
__global__ void update_h(int nx, int ny, int nz, float *ex, float *ey, float *ez, float *hx, float *hy, float *hz, float *chx, float *chy, float *chz) {
	...
	if( idx > nx*ny ) {
		hx[idx] -= chx[idx]*( sz[sidx] - ez_y - sy[sidx] + ey[idx-nx*ny] );
		hy[idx] -= chy[idx]*( sx[sidx] - ex[idx-nx*ny] - sz[sidx] + ez_x );
	}
...
if __name__ == '__main__':
	...
	update_e.prepare("iiiPPPPPPPPP", block=Db, shared=16*16*3*4)
	update_h.prepare("iiiPPPPPPPPP", block=Db, shared=16*16*3*4)
