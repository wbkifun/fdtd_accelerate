...
Bx, By = nz/Dx, (nx-1)*ny/Dy	# number of block
MBy = MAX_BLOCK/Bx
bpg_list = [(Bx,MBy) for i in range(By/MBy)]
if By%MBy != 0: bpg_list.append( (Bx,By%MBy) )
#print bpg_list
bpg0 = (Bx,ny/Dy)	# for asynchronous memcpy
...
for tn in xrange(1, tmax+1):
	...
	update_e.prepared_async_call(bpg0, stream1, np.int32(0), *eh_args)
	for i, bpg in enumerate(bpg_list): update_e.prepared_call(bpg, np.int32(i*MBy+ny/Dy), *eh_args)
	if rank == 0:
		comm.Recv(ey_tmp, 1, 22)
		comm.Recv(ez_tmp, 1, 23)
		cuda.memcpy_htod_async(int(ey_gpu)+(nx-1)*ny*nz*np.nbytes['float32'], ey_tmp, stream1) 
		cuda.memcpy_htod_async(int(ez_gpu)+(nx-1)*ny*nz*np.nbytes['float32'], ez_tmp, stream1) 
		stream1.synchronize()
	elif rank == 1:
		...
		cuda.memcpy_htod_async(int(ey_gpu)+(nx-1)*ny*nz*np.nbytes['float32'], ey_tmp, stream1) 
		cuda.memcpy_htod_async(int(ez_gpu)+(nx-1)*ny*nz*np.nbytes['float32'], ez_tmp, stream1) 
		cuda.memcpy_dtoh_async(ey_tmp, int(ey_gpu), stream1) 
		cuda.memcpy_dtoh_async(ez_tmp, int(ez_gpu), stream1) 
		stream1.synchronize()
