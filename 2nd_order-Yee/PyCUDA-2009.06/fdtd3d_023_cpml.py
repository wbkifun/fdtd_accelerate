...
def initMainArrays( kNtot, devFx, devFy, devFz, initArray ):
	...
	if ( kNtot%TPB == 0 ):
		...
		BPG = kNtot/TPB + 1
	print 'init main arrays: Ntot=%d, TPB=%d, BPG=%d' % (kNtot, TPB, BPG)
...
if __name__ == '__main__':
	...
	BPGpmlx = selectBPG( Npml*Ny*Nz )
	Ntotpmlx = TPBpmlx*BPGpmlx
	print 'BPGpmlx=%d' % (BPGpmlx)

	BPGpmly = selectBPG( Nx*Npml*Nz )
	Ntotpmly = TPBpmly*BPGpmly
	print 'BPGpmly=%d' % (BPGpmly)

	BPGpmlz = selectBPG( Nx*Ny*(Npml+1) )
	Ntotpmlz = TPBpmlz*BPGpmlz
	Ns_pmlz = ( 2*(TPB+1) )*4
	...
	# time loop
	t0 = time()
	for tstep in xrange( 1, 1001 ):
		updateE.prepared_call( (BPGmain,1), kNy, kNz, devEx, devEy, devEz, devHx, devHy, devHz, devCEx, devCEy, devCEz )

		updateCPMLxE.prepared_call( (BPGpmlx,1), kNx, kNy, kNz, devEx, devEy, devEz, devHx, devHy, devHz, devCEx, devCEy, devCEz, psixEyf, psixEzf, 0 )
		...
		updateCPMLxH.prepared_call( (BPGpmlx,1), kNx, kNy, kNz, devEx, devEy, devEz, devHx, devHy, devHz, psixHyf, psixHzf, 0 )
