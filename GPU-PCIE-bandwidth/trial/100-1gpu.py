...
class GpuSpace:
	...
	def get_transfer_rate( s, iterate=10 ):
		dt_htod = sc.zeros( iterate, 'f' )
		dt_dtoh = sc.zeros( iterate, 'f' )
		...
		for i in xrange( iterate ):
			...
			t2 = datetime.now()
			dt = t2 - t1
			dt_htod[i] = dt.seconds + dt.microseconds*1e-6

			t1 = datetime.now()
			cuda.memcpy_dtoh( s.a, s.dev_a )
			t2 = datetime.now()
			dt = t2 - t1
			dt_dtoh[i] = dt.seconds + dt.microseconds*1e-6

		t_rate_htod = s.nbytes/dt_htod.mean()/gbytes
		t_rate_dtoh = s.nbytes/dt_dtoh.mean()/gbytes
