...
__kernel void update_h(int nx, int ny, int nz, __global DTYPE *ex, __global DTYPE *ey, __global DTYPE *ez, __global DTYPE *hx, __global DTYPE *hy, __global DTYPE *hz ARGS_CH) {
	...
	while( idx < nx*ny*nz ) {
		...
		if( j>0 && k>0 ) hx[idx] -= CHX * ((sz[tx] - ez[idx-nz]) - (sy[tx] - sy[tx-1]));
		if( i>0 && k>0 ) hy[idx] -= CHY * ((sx[tx] - sx[tx-1]) - (sz[tx] - ez[idx-ny*nz]));
		if( i>0 && j>0 ) hz[idx] -= CHZ * ((sy[tx] - ey[idx-ny*nz]) - (sx[tx] - ex[idx-nz]));
