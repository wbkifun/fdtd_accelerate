...
__kernel void update_e(int idx0, int nmax, int nx, int ny, int nz, __global DTYPE *ex, __global DTYPE *ey, __global DTYPE *ez, __global DTYPE *hx, __global DTYPE *hy, __global DTYPE *hz ARGS_CE) {
	int tx = get_local_id(0);
	int idx = get_global_id(0) + idx0;
	int i, j, k;
...
__kernel void update_h(int idx0, int nmax, int nx, int ny, int nz, __global DTYPE *ex, __global DTYPE *ey, __global DTYPE *ez, __global DTYPE *hx, __global DTYPE *hy, __global DTYPE *hz ARGS_CH) {
	int tx = get_local_id(0);
	int idx = get_global_id(0) + idx0;
	int i, j, k;
