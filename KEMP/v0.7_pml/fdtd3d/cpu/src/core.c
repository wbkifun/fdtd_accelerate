...
void update_e(int nx, int ny, int nz, DTYPE *ex, DTYPE *ey, DTYPE *ez, DTYPE *hx, DTYPE *hy, DTYPE *hz ARGS_CE) {
	int idx, i, j, k;
	TYPE128 hx0, hy0, hz0, h1, h2, e, INIT_CE, mask_exy={MASK_EXY}, mask_z={MASK};

	OMP_FOR_E
	for( idx=0; idx<nx*ny*nz; idx+=INCRE ) {
        hx0 = LOAD(hx+idx);
        hy0 = LOAD(hy+idx);
        hz0 = LOAD(hz+idx);
...
void update_h(int nx, int ny, int nz, DTYPE *ex, DTYPE *ey, DTYPE *ez, DTYPE *hx, DTYPE *hy, DTYPE *hz ARGS_CH) {
	int idx, i, j, k;
